...
def get_all(isamAppliance, check_mode=False, force=False):
    """
    Retrieving all Client Certificate Mapping
    """
    return isamAppliance.invoke_get("Retrieving all Client Certificate Mapping",
                                    "/wga/client_cert_cdas")


def get(isamAppliance, id, check_mode=False, force=False):
    """
    Retrieve a Client Certificate Mapping
    """
    return isamAppliance.invoke_get("Retrieve a Client Certificate Mapping",
                                    "/wga/client_cert_cdas/{0}".format(id))


def _get_template(isamAppliance):
    """
    Retrieve a Client Certificate Mapping Template
    """
    return isamAppliance.invoke_get("Retrieve a Client Certificate Mapping Template",
                                    "/isam/wga_templates/client_cert_cdas_template")


def add(isamAppliance, id, client_cert_cdas_data=None, check_mode=False, force=False):
    """
    Add a Client Certificate Mapping
    """
    # Add default template if not specified
    if client_cert_cdas_data is None:
        ret_obj = _get_template(isamAppliance)
        client_cert_cdas_data = ret_obj['data']['id']
...
def delete(isamAppliance, id, check_mode=False, force=False):
    """
    Deleting a Client Certificate Mapping
    """
    if force is True or _check(isamAppliance, id) is True:
        if check_mode is True:
            ...
            return isamAppliance.invoke_delete(
                "Delete a Client Certificate Mapping",
                "/wga/client_cert_cdas/{0}".format(id))
...
def rename(isamAppliance, id, new_name, check_mode=False, force=False):
    """
    Rename a Client Certificate Mapping
    """
    if force is True or _check(isamAppliance, id) is True:
        if check_mode is True:
            return isamAppliance.create_return_object(changed=True)
...
def export_file(isamAppliance, id, filename, check_mode=False, force=False):
    """
    Exporting a Client Certificate Mapping
    """
    import os.path
...
def import_file(isamAppliance, id, filename, check_mode=False, force=False):
    """
    Importing a Client Certificate Mapping
    """
...
def _check(isamAppliance, id):
    """
    Check if Client Certificate Mapping already exists
    """
